...
public class SalesReportsApiController : ControllerBase
{
    ...
    private readonly ICustomerBalancePdfService _customerBalancePdfService;
    private readonly ICompanyService _companyService;

    public SalesReportsApiController(
        ISalesReportService salesReportService,
        ICustomerBalancePdfService customerBalancePdfService,
        ICompanyService companyService)
    {
        _salesReportService = salesReportService;
        _customerBalancePdfService = customerBalancePdfService;
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var report = await _salesReportService.GetCustomerBalancesAsync(
                new SalesReportRequest
                {
                    FromDate = fromDate,
                    ToDate = toDate,
    ...
    [HttpGet("customer-balances/pdf")]
    [RequirePermission("Reports.View")]
    public async Task<IActionResult> CustomerBalancesPdf(
        [FromQuery] DateTime fromDate,
        [FromQuery] DateTime toDate,
        [FromQuery] int? customerId,
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var pdf = _customerBalancePdfService.GeneratePdf(
                report,
                company?.CompanyName ?? "Pakistan Accounting ERP");
            var fileName = $"customer-balances-{fromDate:yyyyMMdd}-{toDate:yyyyMMdd}.pdf";
