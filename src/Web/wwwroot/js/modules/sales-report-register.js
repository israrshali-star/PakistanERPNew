(function () {
    ...
    function renderReport(data) {
        $('#report-period').text(
            ...
            (data.customerLabel ? ' — ' + data.customerLabel : '')
        );
        ...
        if (!data.lines || data.lines.length === 0) {
            $tbody.append('<tr><td colspan="10" class="text-muted text-center">No invoices found.</td></tr>');
        ...
        data.lines.forEach(function (line) {
            $tbody.append(
                '<tr>' +
                '<td><code>' + escapeHtml(line.invoiceNumber) + '</code></td>' +
                '<td>' + formatDate(line.invoiceDate) + '</td>' +
                '<td>' + escapeHtml(line.customerName) + '</td>' +
                '<td>' + escapeHtml(line.invoiceType) + '</td>' +
                ...
                '<td>' + (line.fbrInvoiceNumber ? escapeHtml(line.fbrInvoiceNumber) : '—') + '</td>' +
                '</tr>'
            );
        });
    ...
    function loadReport() {
        ...
        $.getJSON('/api/sales-reports/register', params)
            .done(renderReport)
            .fail(function (xhr) {
