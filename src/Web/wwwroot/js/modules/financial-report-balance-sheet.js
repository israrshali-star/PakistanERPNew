(function () {
    ...
    function renderReport(data) {
        $('#report-period').text('As of: ' + formatDate(data.asOfDate));

        var $tbody = $('#report-lines');
        $tbody.empty();

        if (!data.lines || data.lines.length === 0) {
            $tbody.append('<tr><td colspan="4" class="text-muted text-center">No balance sheet balances found.</td></tr>');
        ...
        $('#report-total-assets').text(formatAmount(data.totalAssets));
        $('#report-total-liabilities').text(formatAmount(data.totalLiabilities));
        $('#report-total-equity').text(formatAmount(data.totalEquity));
        $('#report-total-liab-equity').text(formatAmount(data.totalLiabilitiesAndEquity));
